...
(defn error$ [message] "Output error message"
  (notify$ :error message))
(defn warn$ [message] "Output warn message"
  (notify$ :warn message))
(defn info$ [message] "Output info message"
  (notify$ :info message))
(defn debug$ [message] "Output debug message"
  (notify$ :debug message))
(defn trace$ [message] "Output trace message"
...
(defn <-table [from-module message-type] "Constructs message based on the internal table"
  (let [message-table (. (. notifications from-module) message-type)
        notify-type (match message-table.type
                      ...
                      :debug "D"
                      :trace "T")
        output (string.format "kat.nvim %s%s: %s"
                              notify-type message-table.number
