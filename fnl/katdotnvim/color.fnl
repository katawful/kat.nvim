...
(defn- def-back-colors [] "A table of background colors"
       (let [back (if (= (. main.background-mut 1) :dark)
                      (if (= (. main.contrast-mut 1) :soft)
                          ...
                          (or (= (. main.contrast-mut 1) :hard) (= (. main.contrast-mut 1) nil))
                          [dark-fore-back
                           (ucolors.brighten dark-fore-back 0.1)
                           (ucolors.brighten dark-fore-back 0.3)
                           (ucolors.brighten dark-fore-back 0.5)
...
(defn update []
  ...
  (set kat.green.mix_red
       {:desc "Mixes in red" :color (ucolors.blend color*.green color*.red 0.2)})
  ...
  ;; Darker blue
  (set kat.blue.darker
       {:desc "Darkens color by 20%" :color (ucolors.darken color*.blue 0.2)})
  ...
  ;; Red mix red
  (set kat.red.mix_red {:desc "Mixes in red"
                        :color (-> color*.red
  ...
  (let [plum (ucolors.blend color*.pink color*.purple 0.65)]
    (set kat.plum.base {:desc "Base color, mixes purple and pink" :color plum})
    ;; Plum match fg
    ...
    (set kat.plum.mix_meld_bg
         {:desc "Mixes in meld bg" :color (ucolors.blend plum color*.b3 0.2)})
    ;; Plum mix green
    (set kat.plum.mix_green
         {:desc "Mixes in green" :color (ucolors.blend plum color*.green 0.2)}))
  ...
  (set kat.orange.mix_red
       {:desc "Mixes in red" :color (ucolors.blend color*.orange color*.red 0.4)})
  ...
  (set kat.pink.mix_red
       {:desc "Mixes in red" :color (ucolors.blend color*.pink color*.red 0.2)})

  ;; Pink mix blue
  (set kat.pink.mix_blue
       {:desc "Mixes in blue" :color (ucolors.blend color*.pink color*.blue 0.2)})
  ...
  (let [teal (ucolors.blend color*.blue color*.b5 0.65)]
    (set kat.pink.mix_teal
         {:desc "Mixes in teal" :color (ucolors.blend color*.pink teal 0.2)}))
